...
/* 	document.getElementById('razmer_obj').onclick = function() {
		obrez_razm = document.getElementById("razmer_obj").value;
		document.getElementById("razmer_obj").setAttribute("value", obrez_razm)
		localStorage.setItem('razmer_obj', obrez_razm);
		// console.log("obrez: " + obrez)
	} */
...
/* 	if (localStorage.getItem('razmer_obj') > 0) {
		document.getElementById('razmer_obj').value = localStorage.getItem('razmer_obj')
		razmer_obj = localStorage.getItem('razmer_obj')
	} else {
		obrez_razm = document.getElementById("razmer_obj").value;
		document.getElementById("razmer_obj").setAttribute("value", obrez_razm)
		razmer_obj = obrez_razm
		localStorage.setItem('razmer_obj', obrez_razm);
	} */
}
...
function TradingView_11() {
	// TradingV = document.getElementById('TradingView-wrapper')
		...
		// console.log('1: ', pair)
		// var e = document.getElementById("pair");
		// var pair2 = e.textContent || e.innerText;
		// alert(pair2);
	...
	this.soc = function (pair = "EURUSD", size = 60) {
		...
		if ($("#" + $("#pair").attr("val").replace(/[/]/g, '_')).attr("data") !== typ || !wss) {
			...
			if (typ == "OlympTrade" && wss) {
				...
				// if (typeof sub_uid  !== 'undefined') {
					// sub_uid = 1
				// }
				if (!uuid) uuid = (Date.now().toString(36) + Math.random().toString(36).substr(2, 12)).toUpperCase()
				// if (!sub_uid) sub_uid = 1;
				// if(typeof sub_uid === 'undefined'){
				  // alert("sub_uid undefined");
				// }
				// else {
				  // alert("sub_uid defined");
				// }
				// if (!sub_uid) {
...
//добавил округление до 12 знаков
				...
				if (data.ric == 'CRY/IDX') {
					...
					data.rate = Math.round(data.rate * 100000) / 100000
...
var imagess = document.createElement('img');		//создаем картинку
	...
	version_mod.textContent = ' Ver 7.1.0 '
