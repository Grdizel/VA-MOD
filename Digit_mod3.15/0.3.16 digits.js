...
var OddLength, EvenLength, bEvenLength, bOddLength, rEvenLength, rOddLength, dpseborLength, dpsoberLength, temp_Odd=0, temp_Even=0, tdgbEven=0, tdgbOdd=0, tdgrEven=0, tdgrOdd=0, tdpsebor = 0, tdpsober = 0;
...
var dgNumberBlue =[], dgNumberRed = [], dgNumberRedLength, dgNumberBlueLength, tNumberBlue = 0, tNumberRed = 0;
let start = 0;
...
//#################################################################
		...
		for (var i=1; i<cnt+1; i++) {
			...
			if (spot[i-1] < spot[i]) {
				...
				if (spot[i-2] < spot[i]) tNumberBlue+=1
				if (spot[i-2] > spot[i]) tNumberRed+=1
		...
		if (i==20) {
			...
			dgNumberBlueLength = tNumberBlue;
			dgNumberRedLength = tNumberRed;
...
//##############################################################################
				...
				if (dgNumberBlue.length > dgNumberBlueLength){
					dgNumberBlue.shift();
					// console.log(i,digit[i],"синий");
				};
				if (dgNumberRed.length > dgNumberRedLength){
					dgNumberRed.shift();
					// console.log(i,digit[i],"синий");
				};
//##############################################################################
