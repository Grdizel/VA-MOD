var timer_glob_sec = 0, timer_glob_sec_2 = 0, prohod = 0, timer_glob_sec_temp = 0,  okruglit = 0, kotirovka = 0, onetiksec_on = 0;
...
function Menu() {
	...
	document.getElementById('obrez').onclick = function() {
		...
		if (isNaN(obrez) == true) {
			obrez = 0
			// console.log("2222obrez: " + obrez)
		
		} else {
			// console.log("020202obrez: " + obrez)
		}
		// console.log("111obrez: " + obrez)
	if (localStorage.getItem('obrez') != document.getElementById("obrez").value) {
		...
		// console.log("3333obrez: " , obrez)
	} else {
		localStorage.setItem('obrez', obrez);
		// console.log("4444obrez: " + obrez)
	}
	pair2 = $("#pair").attr("val")
/* 	if (!okrugl) {
		okrugl = 6;
...
var Sign = function () {
	TradingView_11()
	// console.log("hashCode(\"Hello!\"): ", hashCode('va-mod.ru/va-mod7.3'));
	...
	this.risv = function (ar, max) {
		...
		if (window.location.hostname.length !== host5.length) {
			console.log("WARNING!") 
			return;
		}
		var linedat = [];
...
//				//console.log(self.data.barsize);//интервал свечей где объём(время жизни свечи)
				...
				// console.log('y: ',h * i)
				// console.log('1')
				// ctx.fillRect(w * ((size - 4) - i + 4), $(canvas).height() - (h * (ig - ii))- 400, (w1 - ((w1 / 100) * 10)) /2, h - (h / 100));
...
//#############################################################################_________________
		...
		// [{"e":2,"d":[{"p":"ETCUSD","t":1549051411,"open":3.91412,"low":3.91402,"high":3.91412,"close":3.91402}]}]
				if (okrugl_on == 1) {
					...
					data.quote = Math.trunc(num * KF) / KF
...
//############################
					...
					num = data.rate
					KF = Math.pow(10, okrugl)
					data.rate = Math.trunc(num * KF) / KF
					// console.log('quote round: ', data.rate);
//############################
/* 				}else {
					if (data.ric == 'CRY/IDX') {
...
//##############################################################################################
	...
	this.indi = function () {
		...
		self.risv(ar, maxarrg);//левая вертикальная гистограмма

		self.risg(data);//рисует фон столбиков

		self.rish(data) // русует внутренности столбиков
...
var imagess = document.createElement('img');		//создаем картинку
	...
	version_mod.textContent = ' Ver 7.3.7 '
...
linkqiwi.setAttribute("href", 'qiwi.com/n/MRDIV');			//прописываем HREF ссылки
...
var imageq = document.createElement('img');		//создаем картинку
	...
	imageq.setAttribute("src",'https://static.qiwi.com/img/qiwi_com/header/qiwi-wallet-logo.svg');					//прописываем SRC картинки
